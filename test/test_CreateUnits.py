...
class TestCreateUnits:
    ...
    def test_creating_units_without_entering_anything(self):
        ...
        #     (By.XPATH, "(//p[normalize-space()='Miami Campus'][1])"))).click()  # clicking on dropdown options
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located((By.XPATH, "//input[@name='unitNum']"))).send_keys(
            )  # entering name
        create_unit.enter_width("")
        create_unit.enter_length("")
        self.driver.implicitly_wait(5)
        create_unit.Not_enter_Sq_Ft()
        create_unit.Not_enter_Maintenance_Fee()
        create_unit.enter_description("")
        create_unit.click_on_available_for_lease_checkbox()
        create_unit.click_on_available_for_sale_checkbox()
        create_unit.enter_lease_price("")
        self.driver.implicitly_wait(3)
        create_unit.enter_buy_price("")
        self.driver.find_element(By.XPATH, "//p[normalize-space()='+Upload images']").click()
        time.sleep(3)
        keyboard = Controller()
        keyboard.type("C:\\Users\\HP\\PycharmProjects\\Luxelocker\\Files\\IMG_0573.jpg")
        ...
        self.driver.implicitly_wait(10)
        call.recallable_save_button(self.driver)
        warning_message_xpath = "Fill out all required fields."
        time.sleep(4)
        assert create_unit.retrieve_warning_message().__contains__(warning_message_xpath)
        print("Fill out all required fields first")
